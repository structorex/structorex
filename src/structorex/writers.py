        ...
        try:
            with open(self.config.output_file, "w", encoding="utf-8") as f:
                ...
                for path, content in visitor.content_dict.items():
                    f.write(f"\n{'='*50}\nFile: {path}\n{'='*50}\n{content}")
            logger.info(
                f"Report successfully saved to: {self.config.output_file}")
...
class MarkdownReportWriter(ReportWriter):
    def write(self, visitor):
        try:
            ...
            logger.info(
                f"Markdown report successfully saved to: {self.config.output_file}")
        except Exception as e:
            logger.error(f"ERROR saving report: {e}")
            raise
...
class JsonReportWriter(ReportWriter):
    def write(self, visitor):
        try:
            ...
            logger.info(
                f"JSON report successfully saved to: {self.config.output_file}")
        except Exception as e:
            logger.error(f"ERROR saving report: {e}")
            raise
    ...
    def write(self, visitor):
        try:
            with open(self.config.output_file, "w", encoding="utf-8") as f:
                ...
                f.write(
                    '#sidebar { width: 350px; overflow-y: auto; background: #f6f8fa; padding: 15px; border-right: 1px solid #e1e4e8; }\n')
                f.write(
                    '#content { flex-grow: 1; padding: 20px; overflow-y: auto; }\n')
                f.write(
                    '.tree-line { font-family: ui-monospace, SFMono-Regular, Consolas, "Liberation Mono", Menlo, monospace; white-space: pre; margin: 2px 0; font-size: 13px; }\n')
                f.write(
                    '.file-link { cursor: pointer; color: #0366d6; text-decoration: none; }\n')
                f.write('.file-link:hover { text-decoration: underline; }\n')
                f.write('.skipped-file { color: #6a737d; }\n')
                f.write(
                    'pre { background: #f6f8fa; padding: 16px; border-radius: 6px; overflow: auto; border: 1px solid #e1e4e8; font-size: 13px; line-height: 1.45; }\n')
                f.write('.file-content-view { display: none; }\n')
                f.write('.file-content-view.active { display: block; }\n')
                f.write('</style>\n</head>\n<body>\n')

                f.write('<div id="sidebar">\n')
                f.write('<h3>Project Structure</h3>\n')
                f.write(
                    f'<p style="font-size: 12px; color: #586069;">Files: {visitor.total_files} | Lines: {visitor.total_lines}</p>\n')
                f.write('<div>\n')
                ...
                for node in visitor.structure_data:
                    ...
                    if node["type"] == "file":
                        ...
                        f.write(
                            f'<div class="tree-line">{prefix}<a class="file-link" onclick="showContent(\'{safe_path}\')">{name}</a></div>\n')
                    elif node["type"] == "skipped":
                        f.write(
                            f'<div class="tree-line"><span class="skipped-file">{prefix}{name} [skipped]</span></div>\n')
                    else:  # dir
                        f.write(
                            f'<div class="tree-line">{prefix}<b>{name}/</b></div>\n')
                ...
                for path, content in visitor.content_dict.items():
                    ...
                    f.write(
                        f'<div id="{safe_path}" class="file-content-view">\n')
                ...
                f.write(
                    '  document.querySelectorAll(".file-content-view").forEach(function(el) { el.classList.remove("active"); });\n')
                ...
                f.write('</body>\n</html>')
            logger.info(
                f"HTML report successfully saved to: {self.config.output_file}")
        except Exception as e:
            logger.error(f"ERROR saving report: {e}")
            raise
