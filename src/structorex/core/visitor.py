...
class ProjectStructureVisitor(FileSystemVisitor):
    ...
    def visit_file(self, file, prefix, is_last):
        ...
        if getattr(file, "skipped", False):
            ...
            self.structure_data.append({"type": "skipped",
                                        "name": file.name,
                                        "path": file.path,
                                        "prefix": prefix,
                                        "connector": connector})
        ...
        self.structure_data.append({"type": "file",
                                    "name": file.name,
                                    "path": file.path,
                                    "prefix": prefix,
                                    "connector": connector})
        ...
        if not self.filter.is_within_size_limit(file.path):
            self.content_dict[file.path] = "[SKIPPED: File too large]"
            return

        if not self.filter.is_text_file(file.path):
            self.content_dict[file.path] = "[SKIPPED: Binary file]"
            return
    ...
    def visit_directory(self, directory, prefix, is_last):
        ...
        self.structure_data.append({"type": "dir",
                                    "name": directory.name,
                                    "path": directory.path,
                                    "prefix": prefix,
                                    "connector": connector})
    ...
    def process_queued_files(self, verbose=False):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future_to_path = {
                executor.submit(
                    read_single_file,
                    path): path for path in self._files_to_read}
